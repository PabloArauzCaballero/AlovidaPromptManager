...
Carril de Pablo, "Corte, laboratorio del piloto y regresión de aislamiento", 2026-09-20:
...
- **El laboratorio de H2** (`AgendaNoticeCapabilityLab`): el contrato real es `AgendaNoticePort`.
  ...
  alguien construyera el validador de contrato de Ender.
- **Aislar `ChatAutoReplies`** (H6): un bug real de MikroORM, ajeno al módulo bajo trabajo, bloqueaba
...
- **Simular la prueba de ausencia de Itzan** (H4): en vez de esperar a que él corriera su prueba,
  se hizo la misma operación —quitar `MessagingModule` de `SchedulingModule`— en una copia
  controlada, se observó qué rompía, y se revirtió. No reemplaza la prueba oficial de Itzan (se
  declara así), pero da un insumo real para decidir en vez de un `TODO` vacío.
...
1. **Prohibido presentar el resultado de un doble como si fuera el resultado del colaborador
   ...
   ausencia simulada por vos no es "la prueba de ausencia de Itzan".
