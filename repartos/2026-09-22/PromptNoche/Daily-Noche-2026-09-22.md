...
| `filter-bar` con proyección `[filter-bar-action]` | Pablo, H3.S3 | primera mitad | | `TODO` |
| `data-table` con alto máximo y sin scroll lateral (opt-in) | Pablo, H3.S2 | | | `TODO` |
| `work-history` con `layout="tabla"` para el historial | Pablo, H4.S3 | segunda mitad | | `TODO` |
...
| **HALL-M5 — contrato de `shell-layout.spec.ts` tras el merge de Cotizaciones** | Justin, H4/H5 | — | `b3af9887` agregó `/my-account/cotizaciones` al menú del paciente; #581 actualizó las dos listas cerradas del spec para incluirla entre órdenes diagnósticas y cuestionarios. Evidencia: `shell-layout.spec.ts` 54/54 y merge `66bbcb76` en `mockup`. | **RESUELTO** |
...
| Justin | 20 / 51 | 0 / 6 | El plan fuente registra 20 microtareas verificadas; #578–#581 están fusionados. #583 publica Playwright 2/2, barrido de rutas 5/5 y simulador 21/21 locales; no se suman por solaparse con los DoD originales. | Siguen faltando medición antes/después y observación del mockup remoto | [Justin-Daily-Noche-2026-09-22.md](Justin/Justin-Daily-Noche-2026-09-22.md) |
