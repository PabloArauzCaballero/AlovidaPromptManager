...
> Pablo 45/68 · Itzan 89/110 (el plan creció de 93 a 110) · Justin 44/51 (cierre 2026-09-24, doble revisión de 6 rondas) · Ender 47/48 · Marcelo 0/59.
...
| **Itzan** | [El perfil sin «principal», sin datos del trabajo, y tres tablas con modal, adjunto, buscador y paginación](Itzan/Noche-PerfilMedico.ConfigurarTuPerfil/PerfilSinPrincipalSinTrabajoYTablasConModal.md) | D-01, D-02, D-03, D-06 (patrón), D-07, D-08 (aplicación), D-09, D-10, N-03 | 8 | 18 | 93 | `EN CURSO` |
...
| Medición del flujo de reserva (peticiones, tiempos) | Justin, H1.S2 | **temprano** | `mantra-core-health` PR de la rama `justin/baseline-historico-reserva-2026-09-23`, detalle en `docs/trabajo/2026-09-23-baseline-comparable-reserva/REPORTE.md`. **Dos escenarios, 10 muestras por corte cada uno**, entre `b7785e36` y la punta de `mockup` `a43ad2b3`. Hasta los cupos de una profesional sintética, con cuatro activaciones: mediana **1428 → 1108 ms**. Camino por defecto, que termina en «Todavía no publicó horarios»: mediana **1460 → 892 ms** *(corregido el 2026-09-24: decía 1691 → 1163 y 1431 → 991, medianas del set anterior a la tercera revisión de #610)*. **Peticiones de negocio observables: 0 en los dos cortes** — el simulador es un interceptor en memoria, así que lo que se ve en la pestaña Red son los `chunk-*.js` del servidor de desarrollo. **Ender, lo que te sirve para decidir latencia:** los rangos se solapan, así que una corrida sola no distingue un corte del otro; lo que sí se sostiene es que **0 de 10** muestras nuevas alcanzan la mediana vieja en el escenario de cupos. Y **la mejora no se adjudica al Directorio**: entre los dos cortes hay 8 merges de PR, incluida tu tabla determinista (`/scheduling/slots` de `120 + random(0..179)` a `40` fijos), que es justo la ruta caliente de elegir médico. Separarlas pide un tercer corte que no se hizo. | **PUBLICADO** (con límites: ver el reporte) |
...
| Renglón «Cotizaciones» PATIENT + ruta · retiro «Mis puntos» + redirect | Ender, H4.S2 | a pedido | «Cotizaciones»: **ya estaba** en `origin/mockup` (Justin, `navigation.map.ts:1210`, `app.routes.ts:205`) — verificado contra código, no recreado; sólo faltaba el spec (ver HALL-M5 arriba). «Mis puntos»: renglón retirado (`fueraDelMenuPara: [ANY_ROLE]`), `/my-account/loyalty` redirige a `/my-account` (`SECCIONES_REDIRIGIDAS` en `app.routes.ts`) — esa noche la pestaña de Itzan no había llegado (contrato simulado, regla 65). Apareció y se corrigió un error real de Angular al conectar el redirect con el guard de rol (`NG04014`). **Actualizado 2026-09-24:** Itzan publicó el contrato (#606, en `mockup`) y la integración final está en el PR **#655** de producto (abierto, sin merge): `/my-account/loyalty` → `/my-account?pestana=puntos`, conservando el query previo, y validada en navegador con paciente y médica. El fallback a `/my-account` quedó reemplazado | **PUBLICADO** (contrato final `?pestana=puntos`, PR #655) |
| `output` del `ubicacion-picker` al tocar el mapa | Itzan, H5.S1 | 23/09 | **`puntoElegido`** (`output<Coordenadas>()`): sale en cada toque sobre el plano; «Usar mi ubicación» y quitar no lo emiten. En `mockup` con el [PR #606](https://github.com/mdavila-2001/mantra-core-health/pull/606) | **PUBLICADO** |
...
| Itzan | 89 / 110 | 3 / 8 | H4.S3.M7, H5.S2.M4, H5.S2.M5, H6.S1.M1–M3, H6.S1.M10, H7.S1.M6. Con #606 y #613 fusionados y el [PR #662](https://github.com/mdavila-2001/mantra-core-health/pull/662) (H1–H4) abierto sin fusionar; H8 sin empezar | ninguno | [Itzan-Daily-Noche-2026-09-22.md](Itzan/Itzan-Daily-Noche-2026-09-22.md) |
...
| Ender | 47 / 48 | 6 / 6 | H4 11/11: H4.S2.M2–M4 cerradas el 2026-09-24 con el contrato `?pestana=puntos` de Itzan (#606) en el PR #655 de producto (abierto, sin merge). La única no hecha es 1 descartada (H2.S2.M2). H3 con el contrato **D-H3-PROV-01 = C**: agenda para 13 profesionales de demostración, ninguna para las personas reales de la planilla. PR #604 de producto MERGED | ninguno propio; falta el merge externo de #655 | [Ender-Daily-Noche-2026-09-22.md](Ender/Ender-Daily-Noche-2026-09-22.md) |
