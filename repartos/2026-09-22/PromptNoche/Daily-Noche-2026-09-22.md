...
| **HALL-M5 — `shell-layout.spec.ts` en rojo tras tu merge de cotizaciones** | Justin, H4/H5 | — | `b3af9887` agregó `/my-account/cotizaciones` al menú del paciente; `shell-layout.spec.ts:147` (lista exacta de rutas) y `:449` (agrupación) no se actualizaron. Reproducido en aislamiento, 2/2 rojo, ya está así en `origin/mockup`. No lo corregí (fuera de mi reserva): lo arregla quien tenga `shell-layout.spec.ts` esta noche o Justin en su cierre. | **A CORREGIR** |
