...
|---|---|---|---|---|
| D1 · Corpus masivo del glosario | **10/11** | `TESTED` · visual `UNKNOWN` | [#704](https://github.com/mdavila-2001/mantra-core-health/pull/704) **mergeado 2026-09-26 01:16 UTC** | 2 289 términos (69 curados + 195 enfermedades + 107 análisis + 1 918 ICD-10-CM); 323 códigos verificados contra el NLM. **Build medido el 26/09: 1,29 MB, igual que antes** (el fixture va en el chunk diferido); docs en PR aparte. Falta sólo capturas de `/glossary` (navegador) |
| D2 · AI service: `glosario.v1` + `/v1/diagnosis/suggest` | **8/8** | **`VERIFIED` en el VPS** (96/96, `docs:check` 0) | [#2](https://github.com/PabloArauzCaballero/AlovidaAIService/pull/2), [#4](https://github.com/PabloArauzCaballero/AlovidaAIService/pull/4) y [#5](https://github.com/PabloArauzCaballero/AlovidaAIService/pull/5) (camino con modelo) **mergeados el 2026-09-26** | **Desplegado en Contabo el 2026-09-26** (`feat/triage-service` @ `7c30ee6`, por SSH con contraseña vía paramiko, sin claves instaladas) y verificado desde afuera: `/v1/glossary/catalog` → 2 289 términos, `glosario-v1-2862369d`; `/v1/diagnosis/suggest` con fiebre+tos+dolor de pecho → `neumonia` 0,74 primero con radiografía de tórax, `source: model` (gemini-3.1-flash-lite, ~1,4 s); triage «ando bajoneada» sigue en `source: model`; `mockup…/ai/v1/diagnosis/suggest` → 200 |
| D3 · Tabla de presuntivos → evidencia → cierre (C3) | **8/8** | **`VERIFIED`** en navegador real (typecheck 0; specs 9 + 38 + 7 + 39 en verde; 9 capturas en `evidencia/d3/`) | [#706](https://github.com/mdavila-2001/mantra-core-health/pull/706) `MERGEABLE` | Entrega C3: `POST /clinical/conditions/:id/verification` real (200/404/409/422), `ClinicalClient.verifyCondition`, tabla en la casilla Diagnóstico y diálogo Confirmar/Rechazar con evidencia (orden, informe o nota). Pendiente: doble revisión de capturas por otra persona; P41 en backend |
| D4 · Cierre del formulario con orden + tentativo (IA) | **4/5** | **`VERIFIED`** en navegador real contra el servicio del VPS (typecheck 0; specs 5 + 8 + 49 en verde; 8 capturas en `evidencia/d4/`) | [#707](https://github.com/mdavila-2001/mantra-core-health/pull/707) `MERGEABLE` | `DiagnosisIaClient` + bloque «Cierre del formulario» (sugerencias con «por qué» y disclaimer, «Usar» resuelve contra el catálogo o avisa) + encadenado cerrar → diagnóstico presuntivo → orden en `specialty-form-block`. Pendiente: doble revisión de capturas; `requestStudy` manda `category` y `categoryConceptId` hasta P40; el 412 de estudio duplicado se cuenta como fallo del paso |
